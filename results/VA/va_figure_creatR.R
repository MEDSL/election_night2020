...
county_results <- subset(county_results, state_po=="VA")
...
#county_results_richmond <- subset(county_results, county=="RICHMOND" | county=="RICHMOND CITY")
#county_results <- subset(county_results, county!="RICHMOND" & county!="RICHMOND CITY" )



###import election 2020 results 
list.files()
va2020 <- read_xlsx("va_results.xlsx", sheet="11051245pm")
va2020$county <- str_to_upper(va2020$county)
va2020$dem_pct2020 <- (va2020$biden/(va2020$biden+va2020$trump))*100
va2020$total2020 <- va2020$biden+va2020$trump
county_results <- merge(county_results,va2020, by="county",all.x=T )
...
##merging data 
county_metro <- subset(county_metro, Geo_STUSAB=="va")
...
demo_results_plot <- ggplot(county_metro2,aes(y=dem_pct2020,x=dem_pct,size=var_size,color=metro_factor,label=county.y)) +
  geom_text(alpha=0.6) + scale_color_manual(values = medsl_brands[c(1:4,6)],drop=F) + theme_minimal() +
  guides(size=FALSE)  + labs(title="Virginia Correlation between Democratic vote share",x="Clinton %",y="Biden %", color="Metro Type",
                             caption = paste0(caption_date , "\nMetro areas categorized via National Center for Health Statistics coding."))+
  scale_y_continuous(labels = function(x) paste0(x, "%"), limits = c(0,100)) + 
  scale_x_continuous(labels = function(x) paste0(x, "%"), limits = c(0,100)) + theme(plot.caption = element_text(hjust=0)) +
...
ggsave(paste0("va_dem_pct_correlation",sep="", ".png"), plot = demo_results_plot, scale = 1,
       width = 9, height = 6, units = c("in"), dpi = 600) 
ggsave(paste0("va_dem_pct_correlationpt",sep="", ".png"), plot = demo_results_plotpt, scale = 1,
       width = 9, height = 6, units = c("in"), dpi = 600) 
