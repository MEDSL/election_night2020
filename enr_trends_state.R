...
nyt_time$dem2party_pct <- (nyt_time$dem/nyt_time$party2vote)*100

#biden needed to win calc
summary(nyt_time$magnitude_diff_overturn)
nyt_time$biden_needed2win <- 50 - (nyt_time$magnitude_diff_overturn)*100
nyt_time$biden_needed2win[nyt_time$biden_needed2win < 0] <- 0
summary(nyt_time$biden_needed2win)
###Let's create the plot here of Michigan

mi_overturn_plot <- ggplot(subset(nyt_time, state=="michigan" & hours.from.close <= 60), aes(x=hours.from.close)) +
  geom_line(aes(y=dem2party_pct,col= "Reported for \nBiden",linetype="Reported for \nBiden") , lwd=1.2) +
  geom_line(aes(y=biden_needed2win, col= "Needed for \nBiden to win",linetype="Needed for \nBiden to win"), lwd=1.2) + 
  geom_line(aes(y=prop2party*100,col="Ballots \nreported",linetype="Ballots \nreported"),lwd=1.2) + theme_minimal() +  
  ylim(0,100) + scale_color_manual("",values = c(medsl_brands[3],medsl_brands[2],medsl_brands[1])) + 
  theme(title = element_text(size = rel(1.2), family="Styrene B"), 
        plot.caption = element_text(hjust=0),panel.grid.minor = element_blank(),
        axis.text.x = element_text(angle = 45,vjust=0.5),legend.position = 'bottom', 
        legend.spacing.x = unit(1.0, 'cm')) + 
  scale_linetype_manual(name = "", values = c(1,3,1)) +
  labs(title="Michigan",
       x="Hours from polls closing",y="%",
       caption = caption_date)
mi_overturn_plot
ggsave("results/plots/biden_mi_plot.png", plot = mi_overturn_plot, scale = 1,
